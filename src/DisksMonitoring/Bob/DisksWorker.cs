...
class DisksWorker
{
    ...
    private async Task<(List<VolumeWithBob> toStart, List<VolumeWithBob> toStop)> GetDisksFromBob(
        BobApiClient bobApiClient,
        IEnumerable<VolumeWithBob> volumesWithBob
    )
    {
        ...
        else
        {
            ...
            foreach (var volumeWithBob in volumesWithBob)
            {
                var inactiveInBob = inactiveDiskNames.Contains(volumeWithBob.BobDisk.DiskNameInBob);
    ...
    )
    {
        ...
        if (retry == configuration.StartRetryCount + 1)
            ...
            logger.LogInformation("Successfully changed state of {Disk} to {State}", disk, state);
    }
